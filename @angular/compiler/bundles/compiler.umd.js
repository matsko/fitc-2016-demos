...
(function (global, factory) {
  ...
  var CompileAnimationQueryMetadata = (function (_super) {
      ...
      function CompileAnimationQueryMetadata(selector, animation) {
          _super.call(this);
          this.selector = selector;
  ...
  var AnimationQueryAst = (function (_super) {
      ...
      function AnimationQueryAst(selector, propName, animation) {
          _super.call(this);
          this.selector = selector;
  ...
  function _parseTransitionAnimation(data, entryMetadata, currentTime, collectedStyles, stateStyles, errors) {
      ...
      if (data instanceof CompileAnimationQueryMetadata) {
          ...
          ast = new AnimationQueryAst(data.selector, queryPropName, _parseTransitionAnimation(innerData, entryMetadata, currentTime, collectedStyles, stateStyles, errors));
      }
      ...
      boundProps.forEach(function (boundProp) {
          ...
          switch (boundProp.type) {
              ...
              case exports.PropertyBindingType.Animation:
                  ...
                  var queryExprs_1 = [];
                  var entry = view.animationsSummaryMap[animationName];
                  if (entry && entry.queries) {
                      entry.queries.forEach(function (query) {
                          queryExprs_1.push(THIS_EXPR.prop(query.propName));
                      });
                  }
                  var queryVariable = variable('elementQueries');
                  var queryStmt = queryVariable.set(literalArr(queryExprs_1)).toDeclStmt();
                  updateStmts.push(queryStmt);
                  view.detachMethod.addStmt(queryStmt);
                  updateStmts.push(animationFnExpr.callFn([THIS_EXPR, renderNode, queryVariable, oldRenderVar, newRenderVar]).toStmt());
                  view.detachMethod.addStmt(animationFnExpr.callFn([THIS_EXPR, renderNode, queryVariable, oldRenderValue, emptyStateValue])
                      .toStmt());
                  break;
          }
  ...
  var CompileMetadataResolver = (function () {
      ...
      CompileMetadataResolver.prototype.getAnimationMetadata = function (value) {
          ...
          if (value instanceof _angular_core.AnimationWithStepsMetadata) {
              ...
              var steps = value.steps.map(function (step) { return _this.getAnimationMetadata(step); });
              return new CompileAnimationQueryMetadata(value.selector, steps);
          }
          return null;
      };
